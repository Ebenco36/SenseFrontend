      ...
      <v-container fluid class="pa-6">
        ...
        <v-card border flat>
          ...
          <v-data-table-server
            ...
            :headers="headers"
            :items-length="pagination.total_records"
            :items="serverItems"
            :loading="loading"
            class="elevation-0"
          ...
          >
            ...
            <template v-slot:item.title="{ item }">
              <a href="#" @click.prevent="navigateToDetails(item.primary_id)" class="font-weight-medium result-title" :title="item.title">
                {{ truncateText(item.title, 100) }}
              </a>
            </template>
            
            <template v-slot:item.link="{ item }">
              <v-btn size="small" variant="text" color="primary" :href="item.link" target="_blank" @click.stop>Link</v-btn>
            </template>
            
            <template v-slot:item.overallConf="{ item }">
              <v-chip :color="getConfidenceColor(item.overallConf)" size="small" variant="tonal" class="font-weight-bold">{{ item.overallConf }}</v-chip>
            </template>
              ...
              <tr>
                <td :colspan="columns.length">
                  <v-card class="ma-2 pa-10" flat>
                      ...
                      <v-row>
                        <v-col cols="12" md="6">
                          <div class="detail-item"><span class="detail-label">Diseases:</span><span class="detail-value">{{ item.disease }}</span></div>
                          <div class="detail-item mt-2"><span class="detail-label">Open Access:</span><span class="detail-value">{{ item.openAccess }}</span></div>
                        </v-col>
                        <v-col cols="12" md="6">
                          <div class="detail-item"><span class="detail-label">Studies Included:</span><span class="detail-value">{{ item.numStudies }}</span></div>
                           <div class="detail-item mt-2"><span class="detail-label">Country Focus:</span><span class="detail-value">{{ item.country }}</span></div>
                        </v-col>
                      </v-row>
                  </v-card>
                </td>
          ...
          <div v-if="viewMode === 'list'">
            ...
            <div v-else>
              <v-list lines="three">
                ...
                >
                  <v-list-item-title class="text-h6 font-weight-bold text-blue-darken-4 result-title">{{ item.title }}</v-list-item-title>
                  <v-list-item-subtitle class="text-body-2 text-medium-emphasis my-2">{{ item.authors }} ({{ item.year }})</v-list-item-subtitle>
                  <p class="text-caption text-grey-darken-1 mt-1">{{ truncateText(item.abstract, 400) }}</p>
                  <template v-slot:append>
                    <div class="d-flex flex-column align-end ga-2">
                      <v-chip :color="getConfidenceColor(item.overallConf)" size="small" variant="tonal" class="font-weight-bold">{{ item.overallConf }}</v-chip>
                      <v-btn size="small" variant="tonal" color="primary" :href="item.link" target="_blank" @click.stop>Details</v-btn>
                    </div>
                  </template>
                </v-list-item>
              </v-list>
              <v-pagination v-if="pagination.total_pages > 1" v-model="pagination.current_page" :length="pagination.total_pages" :total-visible="7" class="my-4"></v-pagination>
            </div>
          </div>
        </v-card>
      </v-container>
...
interface ServerItem {
  ...
  year: number | string; country: string; numStudies: number | string; litSearchDate: string; 
  publication_type: string; pdf_url: string; openAccess: string; disease: string; overallConf: string;
  amster2Flaws: string | number;
}
...
const currentSortBy = ref<SortItem[]>([]);
...
const loadItems = async () => {
  ...
  try {
    ...
    serverItems.value = (apiData?.records || []).map((record: any): ServerItem => ({
      ...
      numStudies: record.number_of_references,
      litSearchDate: record.date,
...
const headers = [
  ...
  { title: 'Date of Search', key: 'litSearchDate', align: 'start', sortable: true },
  { title: 'Quality', key: 'overallConf', align: 'start', sortable: true },
  { title: 'AMSTAR 2 Met', key: 'amster2Flaws', align: 'start', sortable: true },
];
...
<style scoped>
.result-title { color: #153a9d; text-decoration: none; cursor: pointer; }
.result-title:hover { text-decoration: underline; }
.list-item-link:hover { background-color: rgba(0, 0, 0, 0.04); }
