                ...
                <v-card-text>
                    <form @submit.prevent="submitFilters">
                        <div class="categories-container">
                            ...
                            <div v-for="(categories, categoryKey) in filters.tag_filters" :key="categoryKey"
                                class="category">
                                <h3>{{ categoryKey }}</h3>
                                <div class="add-text">
                                ...
                                <div class="groups-container">
                                    <div v-for="(group, groupKey) in categories" :key="groupKey" class="group">
                                        <small v-if="isPopulationOrIntervention(categoryKey)">
                                            {{ replace__with_space("", groupKey) }}
                                        </small>
                                        <div class="options-container">
                                            <div v-for="(option, optionKey) in group" :key="optionKey" class="option">
                                                <label>
                                                    <input type="checkbox"
                                                        :checked="selectedFilters.includes(option.display) || (option.synonyms ?? []).some((synonym) => selectedFilters.includes(synonym))"
                                                        @change="toggleOption(option, $event.target.checked)" />
                                                    {{ replace__with_space(categoryKey, option.display) }}
                                                </label>
                            ...
                            <div v-for="(items, otherKey) in filters.others" :key="otherKey" class="category">
                                ...
                                <div class="add-text">
                                    <input type="text" v-model="newOtherText[otherKey]"
                                        placeholder="Add custom item and press Enter"
                                ...
                                <v-col>
                                    <div class="additional-fields">
                                        ...
                                        <div v-for="(field, index) in additionalFields" :key="index" class="field-row">
                                            ...
                                            <button v-if="index" type="button" @click="removeField(index)">
                                                Remove
                                            </button>
                                            <button type="button" @click="addField"
                                                v-if="index === additionalFields.length - 1">
                                                Add
                                            </button>
                                        </div>
                                    </div>
                                        ...
                                        <v-col cols="6" class="field-row-2nd">
                                            <label>
                                                ...
                                                <select v-model="orderBy.column" style="width: 100%!important">
                                                    <option v-for="column in orderColumns" :key="column"
                                                        :value="column">
                                                        {{ column }}
                                                    </option>
                                                </select>
                        ...
                        <div class="d-flex justify-end mt-3">
                            <v-btn text color="primary" @click="closeModal" class="mr-2">
                                Close
                            </v-btn>
...
const loadFilters = async () => {
    try {
        ...
        if (response.status === 'success') {
            ...
            Object.entries(filters.value.tag_filters).forEach(([categoryKey, category]) => {
            ...
            const sortedCategories = categoryOrder.reduce((sorted, key) => {
...
const toggleOption = (option: { display: string; synonyms?: string[] }, isChecked: boolean) => {
    if (isChecked) {
        ...
        selectedFilters.value = selectedFilters.value.filter(
            (item) => !((option.synonyms ?? [option.display]).includes(item))
        );
    }
};
...
const reorderOthers = (others: FiltersData['others']): FiltersData['others'] => {
    const reordered = {} as FiltersData['others'];
...
const isPopulationOrIntervention = (key: string) =>
    ['Population', 'Intervention'].includes(key);
...
const addTextToOther = (otherKey: string) => {
    ...
    if (!filters.value.others[otherKey]) filters.value.others[otherKey] = [];
    filters.value.others[otherKey].push(text);

    // Track the item as custom
    if (!customOthers[otherKey]) customOthers[otherKey] = new Set();
...
const removeOtherItem = (otherKey: string, index: number) => {
    const item = filters.value.others[otherKey][index];

    // Remove the item from `filters.others`
    filters.value.others[otherKey].splice(index, 1);

    // If the item is custom, remove it from `customOthers`
    if (customOthers[otherKey]) {
        customOthers[otherKey].delete(item);
    }
};
...
const submitFilters = () => {
    ...
    const othersFields = Object.keys(filters.value.others).reduce((acc, otherKey) => {
        const selectedForField = filters.value.others[otherKey].filter((item) =>
            selectedFilters.value.includes(item)
        );
    ...
    const filteredUserSelections = selectedFilters.value.filter(
        (item) => !othersItems.includes(item)
    );
...
const replace__with_space = (category: string, input: string) =>
    category === 'Population'
        ? input.replace(/__/g, '-').replace(/_/g, ': ')
        : input.replace(/__/g, ' ');
